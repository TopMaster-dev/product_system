...
async def test_collect_diffs_for_master_without_snapshot(db_session: AsyncSession) -> None:
    ...
    db_session.add(sku)
    await db_session.commit()
    diffs, _summary = await collect_diffs({"NEW-1": 50})
    assert len(diffs) == 1
...
async def test_full_run_persists_reconcile_run_with_diffs(
    ...
    exit_code = await run(csv_path, triggered_by="test-runner")
    assert exit_code == 0

    runs = (await db_session.execute(select(ReconcileRun))).scalars().all()
    assert len(runs) == 1
    run_row = runs[0]
    ...
    assert len(diffs) == 1
    assert diffs[0].master_sku_id == sku_b.id
    assert diffs[0].delta == 2


@pytest.mark.integration
@pytest.mark.asyncio
async def test_dry_run_does_not_persist(db_session: AsyncSession, tmp_path: Path) -> None:
    sku = MasterSku(sku_code="DRY-A", name="A", attributes={})
    db_session.add(sku)
    await db_session.flush()
    db_session.add(InventorySnapshot(master_sku_id=sku.id, on_hand_qty=2))
    await db_session.commit()
    ...
    exit_code = await run(csv_path, triggered_by="dry", dry_run=True)
    assert exit_code == 0
    runs = (await db_session.execute(select(ReconcileRun))).scalars().all()
    assert runs == []
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_missing_csv_returns_exit_2(tmp_path: Path) -> None:
    exit_code = await run(tmp_path / "does-not-exist.csv")
    assert exit_code == 2
